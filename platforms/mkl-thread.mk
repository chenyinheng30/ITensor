### 平台参数文件: Intel MKL 2023.0.0, LP64 + THREADED (libmkl_intel_thread + libiomp5)
###
### 用途
###   覆盖 ITensor/options.mk, 然后重建 ITensor 与本工程:
###
###     cp platforms/mkl-thread.mk ITensor/options.mk
###     cd ITensor && make clean && make build && make cmake-package
###     cd .. && cmake --build build -j
###
###   运行前先 export MKL_NUM_THREADS=4 / OMP_NUM_THREADS=1.
###   bench_* 程序会把当前环境变量打印在结果表上方.
###
### 说明
###   LP64          : 与 ITensor 的 using LAPACK_INT = MKL_INT (= int) 一致
###   intel_thread  : MKL 自带多线程内核, 用 Intel OpenMP (libiomp5) 做并行
###   libiomp5      : MKL 的 lib 目录里没有它, 在 Intel 编译器运行时目录下
###
###   ITENSOR_USE_OMP 必须保持关闭: 否则 libiomp5 与 ITensor 自带的
###   OpenMP 并行区间会互相抢占, 实测比单线程更慢.
###
...
IOMPROOT=/opt/intel/compiler/2023.0.0/linux/compiler
BLAS_LAPACK_INCLUDEFLAGS=-I$(MKLROOT)/include
BLAS_LAPACK_LIBFLAGS=-L$(MKLROOT)/lib/intel64 -L$(IOMPROOT)/lib/intel64_lin \
